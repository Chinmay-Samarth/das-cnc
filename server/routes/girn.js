...
router.get('/', verifyEmployeeAuth, async (req, res) => {
  try {
    ...
    const girns = (data || []).map((g) => ({
      ...
      purchase_order_id: g.purchase_order_id ?? g.purchase_orders?.id ?? null,
      purchase_order_number: g.purchase_orders?.po_number ?? null,
      csr: g.csr,
  ...
  try {
    ...
    return res.json({
      girn: {
        ...
        purchase_order_id: girn.purchase_order_id ?? girn.purchase_orders?.id ?? null,
        purchase_order_number: girn.purchase_orders?.po_number ?? null,
